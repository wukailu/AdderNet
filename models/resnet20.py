...
from models.utils import record_feature, obj_record_feature, append_with_feature
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10):
        ...
        self.avgpool = obj_record_feature(nn.AvgPool2d(8, stride=1), record=False)
    ...
    def forward(self, x):
        ...
        f_list, x = self.avgpool(x)
        x = self.fc(x)
        x = self.bn2(x)

        return f_list, x.view(x.size(0), -1)
